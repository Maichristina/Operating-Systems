...
*/
 ...
 /*
 if someone writer is waiting, send him a signal to start
 */
    else if (wait_w > 0){
...
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    pthread_mutex_lock(&glob_lock); /**/
    act_r--; /*  readers is not active*/
    /*
    if we dont have active readers but we have at least one(or and more)
    waiting writer, send signal one to start
    */
    if (act_r == 0 && wait_w > 0){
        pthread_cond_signal(&writer_wait); /**/
    }
